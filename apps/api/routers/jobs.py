...
@router.get(
    "/{job_id}/diagnostic",
    summary="Diagnostic information for a job",
    description="Get detailed diagnostic information about a job and its file status"
)
async def get_job_diagnostic(job_id: str):
    """
    Get diagnostic information about a job including file validation
    """
    try:
        job_manager = JobManager()
        job = await job_manager.get_job(job_id)
        
        # Check file existence for each input file
        from app.services.storage import StorageService
        storage_service = StorageService()
        ...
        missing_files = len(file_status) - existing_files
        filename_only_count = sum(1 for f in file_status if f["type"] == "filename_only")
        ...
        return {
            ...
            "summary": {
                ...
                "missing_files": missing_files,
                "filename_only_entries": filename_only_count,
                "can_process": existing_files == len(file_status)
            },
            "file_details": file_status,
            "recommendations": [
                "Use POST /{job_id}/retry to retry failed jobs with existing files",
                "Re-upload files if they are missing from Azure Blob Storage",
                "Create a new job if too many files are missing"
            ]
        }
        ...
        logger.error(f"Failed to get diagnostic info: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to get diagnostic information")
